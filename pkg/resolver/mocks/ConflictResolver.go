...
// ResolveConflict provides a mock function with given fields: universe, r, imports, imp, symbol, from
func (_m *ConflictResolver) ResolveConflict(universe resolver.Universe, r *rule.Rule, imports resolver.ImportMap, imp *resolver.Import, symbol *resolver.Symbol, from label.Label) (*resolver.Symbol, bool) {
	ret := _m.Called(universe, r, imports, imp, symbol, from)

	var r0 *resolver.Symbol
	if rf, ok := ret.Get(0).(func(resolver.Universe, *rule.Rule, resolver.ImportMap, *resolver.Import, *resolver.Symbol, label.Label) *resolver.Symbol); ok {
		r0 = rf(universe, r, imports, imp, symbol, from)
	} else {
		if ret.Get(0) != nil {
	...
	var r1 bool
	if rf, ok := ret.Get(1).(func(resolver.Universe, *rule.Rule, resolver.ImportMap, *resolver.Import, *resolver.Symbol, label.Label) bool); ok {
		r1 = rf(universe, r, imports, imp, symbol, from)
	} else {
		r1 = ret.Get(1).(bool)
	}
