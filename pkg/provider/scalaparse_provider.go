...
// NewScalaparseProvider constructs a new NewScalaparseProvider.
func NewScalaparseProvider(progress progressFunc) *ScalaparseProvider {
	return &ScalaparseProvider{
		progress:   progress,
...
// ScalaparseProvider is provider for scala source files. If
...
type ScalaparseProvider struct {
...
func (r *ScalaparseProvider) Name() string {
	return "scalaparse"
}

// RegisterFlags implements part of the resolver.KnownImportProvider interface.
func (r *ScalaparseProvider) RegisterFlags(flags *flag.FlagSet, cmd string, c *config.Config) {
}

// CheckFlags implements part of the resolver.KnownImportProvider interface.
func (r *ScalaparseProvider) CheckFlags(flags *flag.FlagSet, c *config.Config, importRegistry resolver.KnownImportRegistry) error {
	r.importRegistry = importRegistry
	return nil
}

// OnResolve implements part of the resolver.KnownImportProvider interface.
func (r *ScalaparseProvider) OnResolve() {
	r.parser.Stop()
}

// CanProvide implements the resolver.KnownImportProvider interface.
func (cr *ScalaparseProvider) CanProvide(from label.Label, ruleIndex func(from label.Label) (*rule.Rule, bool)) bool {
...
func (r *ScalaparseProvider) Start() error {
...
func (r *ScalaparseProvider) ParseScalaFiles(from label.Label, kind string, dir string, srcs ...string) (*sppb.Rule, error) {
	rule := &sppb.Rule{
		Label: from.String(),
		Kind:  kind,
...
func (r *ScalaparseProvider) parseScalaFile(dir, filename string) (*sppb.File, error) {
	if filename == "" {
...
func (r *ScalaparseProvider) ProvidedRules() []*sppb.Rule {
...
func (r *ScalaparseProvider) ProvideRule(rule *sppb.Rule) error {
...
func (r *ScalaparseProvider) provideFile(from label.Label, rule *sppb.Rule, file *sppb.File) error {
