package scala

import (
	"fmt"
	"testing"

	"github.com/google/go-cmp/cmp"

	sppb "github.com/stackb/scala-gazelle/build/stack/gazelle/scala/parse"
	"github.com/stackb/scala-gazelle/pkg/resolver"
	"github.com/stackb/scala-gazelle/pkg/testutil"
)

func TestScalaScope(t *testing.T) {
	for name, tc := range map[string]struct {
		known   []*resolver.Symbol
		want    map[string]*resolver.Symbol
		wantErr error
	}{
		"missing java.lang": {
			wantErr: fmt.Errorf("java.lang symbols have not been provided"),
		},
		"missing scala": {
			known: []*resolver.Symbol{
				{
					Type:     sppb.ImportType_CLASS,
					Name:     "java.lang.String",
					Provider: "java",
				},
			},
			wantErr: fmt.Errorf("scala symbols have not been provided"),
		},
		"resolves expected": {
			known: []*resolver.Symbol{
				{
					Type:     sppb.ImportType_PACKAGE,
					Name:     "java.lang",
					Provider: "java",
				},
				{
					Type:     sppb.ImportType_CLASS,
					Name:     "java.lang.String",
					Provider: "java",
				},
				{
					Type:     sppb.ImportType_PACKAGE,
					Name:     "scala",
					Provider: "java",
				},
				{
					Type:     sppb.ImportType_CLASS,
					Name:     "scala.Any",
					Provider: "java",
				},
			},
			want: map[string]*resolver.Symbol{
				"java.lang.String": {
					Type:     sppb.ImportType_CLASS,
					Name:     "java.lang.String",
					Provider: "java",
				},
				"String": {
					Type:     sppb.ImportType_CLASS,
					Name:     "java.lang.String",
					Provider: "java",
				},
				"Any": {
					Type:     sppb.ImportType_CLASS,
					Name:     "scala.Any",
					Provider: "java",
				},
				// "_root_.java.lang.String": {
				// 	Type:     sppb.ImportType_CLASS,
				// 	Name:     "java.lang.String",
				// 	Provider: "java",
				// },
				// "_root_.scala.Any": {
				// 	Type:     sppb.ImportType_CLASS,
				// 	Name:     "scala.Any",
				// 	Provider: "java",
				// },
				// "_root_.String": nil,
				// "_root_.Any":    nil,
				// "Nope":          nil,
			},
		},
	} {
		t.Run(name, func(t *testing.T) {
			known := resolver.NewTrieScope()
			for _, symbol := range tc.known {
				if err := known.PutSymbol(symbol); err != nil {
					t.Fatal(err)
				}
			}
			scala, err := newScalaScope(known)
			if testutil.ExpectError(t, tc.wantErr, err) {
				return
			}
			got := make(map[string]*resolver.Symbol)
			for name := range tc.want {
				sym, _ := scala.GetSymbol(name)
				got[name] = sym
			}
			if diff := cmp.Diff(tc.want, got); diff != "" {
				t.Errorf("Parse (-want +got):\n%s", diff)
			}
		})
	}
}
